...
async function createCard(characterInfo, user) {
    ...
    const profileBaseLayer = await Canvas.loadImage("./src/images/profileBaseLayer.png");
    ...
    const poseidonCabin = await Canvas.loadImage("./src/images/secondLayer/poseidon.png");
    const demeterCabin = await Canvas.loadImage("./src/images/secondLayer/deméter.png");
    const hadesCabin = await Canvas.loadImage("./src/images/secondLayer/hades.png");
    ...
    context.fillText(`${hpCurrent}/${hpBase}HP`, 52 + (barWidth / 2) - (textWidth / 2), 552);
    ...
    context.fillText(`${xpCurrent}/${xpBase}xp`, 309 + (barWidth / 2) - (textWidth / 2), 552);

    //Dólares
    context.font = "24px GG Sans Medium";
    context.fillText(
        `$${characterInfo.info.money}`,
        103,
        625
    );
    //Dracmas
    context.fillText(
        `${characterInfo.info.dracmas} dracma${characterInfo.info.dracmas > 1 || characterInfo.info.dracmas == 0 ? "s" : ""}`,
        355,
        625
    );
    ...
    //-------------------------------------------------------------------------
    const { atrPoints, atrCON, atrFOR, atrAGI, atrINT, atrSAB, atrCAR } = characterInfo.stats;
