  ...
  run: async ({ interaction }) => {
    ...
    const { user } = interaction;
        ...
        async function statusCode(characterName) {
            const oldData = await characterDB.findOne({ userID: user.id, "info.name": characterName });
            ...
            const newData = await characterDB.findOneAndUpdate(
                ...
                {
                    $set: {
                        "info.hitPoints.base": (base + atrCON) + ((charLevel * (bonusPerLvl + atrCON))),
                        "info.hitPoints.current": hitPoints.current > ((base + atrCON) + ((charLevel * (bonusPerLvl + atrCON)))) || hitPoints.current == hitPoints.base ? (base + atrCON) + ((charLevel * (bonusPerLvl + atrCON))) : hitPoints.current,
                    },
                },
                {
            ...
            const attachment = new AttachmentBuilder(statusCard, { name: `statusCard.png` }); //Transformando o buffer do card em attachment.
                    ...
                    components: [
                        {
                            ...
                            options: [
                                ...
                                {
                                    label: "Destreza (DES)",
                                    description: "Altere os valores da Destreza de seu personagem.",
                                    value: "editAtrDES",
                                    emoji: "🦿",
                                },
                                {
            ...
            collectorEditInfo.on("collect", async (interaction) => {
                ...
                interaction.awaitModalSubmit({
                    ...
                    const newNicknames = modalInt.fields.getTextInputValue("nicknamesInput");
            ...
            const handleAttributeEdit = async (interaction, user, character, attribute) => {
                ...
                interaction.awaitModalSubmit({
                    ...
                    const quantityToAdd = Number(modalInt.fields.getTextInputValue(`quantity${attribute}`));
            ...
            collectorAtr.on("collect", async (interaction) => {
                ...
                await handleAttributeEdit(interaction, user, characterName, selectedAttribute);
            });
        }

        //Se o usuário não possuir personagens registrados, execute isto.
        if (userCharactersList.length == 0) return interaction.editReply("Este usuário não possui personagens registrados.")
