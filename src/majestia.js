...
const utils = {
  newCommand: (command) => {
    switch (command.type) {
      case "@interaction":
        ...
        if (
          command.interaction.customId == command.response ||
          command.interaction.values[0] == command.response
        ) {
          command.callback(command);
        }
        break;
      default:
        try {
          command.args = command.message.content
            .replace(/\s+/g, " ")
            .split(" ");
  ...
  mongo: {
    ...
    set: async (path, key, newList, options) => {
      let model = require(path);
      return await model
        .findOneAndUpdate(key, newList, options, (data, doc) => {
          return doc;
        })
        .clone();
    },
